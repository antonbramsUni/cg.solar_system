...
namespace utils {
	...
	texture_object create_texture_object(pixel_data const & tex) {
		...
		glBindTexture(out.target, out.handle);
		glTexParameteri(out.target, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameteri(out.target, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		GLenum internal_format = 
			tex.channels == GL_RED  ? GL_R8    :
			tex.channels == GL_RG   ? GL_RG8   :
			tex.channels == GL_RGB  ? GL_RGB8  :
			tex.channels == GL_RGBA ? GL_RGBA8 : GL_NONE;
		glTexImage2D(
			out.target, 0, GLint(internal_format), 
			tex.width, tex.height, 0, 
			tex.channels, tex.channel_type, 
			tex.ptr());
		return out;
	}
